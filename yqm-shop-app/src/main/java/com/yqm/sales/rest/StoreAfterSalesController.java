...
public class StoreAfterSalesController {
    ...
    @Autowired
    private YqmExpressService yqmExpressService;
    ...
    public ApiResult<List<YqmStoreOrderCartInfoVo>> checkOrderDetails(@PathVariable String key) {
    ...
    public ApiResult<List<StoreAfterSalesVo>> detail(@PathVariable String key) {
        ...
        if (ObjectUtil.isNull(storeInfo)) {
            throw new YqmShopException("售后订单不存在");
        }
        return ApiResult.ok(storeInfo);
    }
    ...
    public ApiResult<List<YqmExpress>> getYqmExpresss(YqmExpressQueryCriteria criteria) {
        return ApiResult.ok(yqmExpressService.queryAll(criteria));
    }
