...
import static com.yqm.utils.FileUtil.transformStyle;
...
public class CreatShareProductService {
	 ...
	 */
	public YqmStoreOrderQueryVo handleQrcode(YqmStoreOrderQueryVo storeOrder,String path){
		//门店
		if(OrderInfoEnum.SHIPPIING_TYPE_2.getValue().equals(storeOrder.getShippingType())){
			...
			storeOrder.setSystemStore(systemStoreService.getYqmSystemStoreById(storeOrder.getStoreId()));
		}
	 ...
	 * @return url
	 */
	public String getSpreadUrl(String from,YqmUser userInfo,String siteUrl,
		...
		if(CharSequenceUtil.isNotBlank(from) && AppFromEnum.APP.getValue().equals(from)){
			...
			if(CharSequenceUtil.isBlank(from)) {
			...
			if(ObjectUtil.isNull(attachment)){
				...
				if(from.equals(AppFromEnum.ROUNTINE.getValue()) || from.equals(AppFromEnum.TARO_MINI.getValue())){
					...
					siteUrl =  CharSequenceUtil.isNotBlank(uniUrl) ? uniUrl :  ShopConstants.DEFAULT_UNI_H5_URL;
	 ...
	 * @return url
	 */
	public String getBargainPosterUrl(Long bargainId, YqmUser userInfo,String siteUrl,
			String apiUrl,String path,String from){
		Long uid = userInfo.getUid();
		YqmStoreBargain storeBargainQueryVo = storeBargainService
				.getById(bargainId);
	 ...
	 * @return url
	 */
	public String getPinkPosterUrl(Long pinkId, YqmUser userInfo,String siteUrl,
			String apiUrl,String path,String from){
		Long uid = userInfo.getUid();
