 ...
 */
package com.yqm.tools.service.impl;
...
import com.yqm.tools.service.mapper.VerificationCodeMapper;
...
public class VerificationCodeServiceImpl extends BaseServiceImpl<VerificationCodeMapper, VerificationCode>  implements VerificationCodeService {
    ...
    public EmailVo sendEmail(VerificationCode code) {
        ...
        if(verificationCode == null){
            ...
            emailVo = new EmailVo(Collections.singletonList(code.getValue()),"yqm-shop后台管理系统",content);
            this.save(code);
            timedDestruction(code);
        // 存在就再次发送原来的验证码
        } else {
            content = template.render(Dict.create().set("code",verificationCode.getCode()));
            emailVo = new EmailVo(Collections.singletonList(verificationCode.getValue()),"yqm-shop后台管理系统",content);
        }
        return emailVo;
    }
