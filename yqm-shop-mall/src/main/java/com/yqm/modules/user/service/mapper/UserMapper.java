...
*/
@Repository
public interface UserMapper extends CoreMapper<YqmUser> {
    ...
    @Select("<script>SELECT u.uid,u.nickname,u.avatar,DATE_FORMAT(u.create_time,'%Y/%m/%d') as time," +
            ...
            "IFNULL(SUM(o.pay_price),0) as numberCount FROM yqm_user u " +
            "LEFT JOIN yqm_store_order o ON u.uid=o.uid " +
    ...
    @Update("update yqm_user set now_money=now_money-#{payPrice}" +
            " where uid=#{uid}")
    int decPrice(@Param("payPrice") BigDecimal payPrice, @Param("uid") Long uid);
//
//    @Update("update yqm_user set brokerage_price=brokerage_price+#{brokeragePrice}" +
//            " where uid=#{uid}")
//    int incBrokeragePrice(@Param("brokeragePrice") double brokeragePrice,@Param("uid") int uid);

    @Update("update yqm_user set pay_count=pay_count+1" +
            " where uid=#{uid}")
    int incPayCount(@Param("uid") Long uid);

    @Update("update yqm_user set now_money=now_money+#{price}" +
            " where uid=#{uid}")
    int incMoney(@Param("uid") Long uid,BigDecimal price);

    @Update("update yqm_user set integral=integral-#{integral}" +
            " where uid=#{uid}")
    ...
    @Update( "update yqm_user set status = #{status} where uid = #{id}")
    void updateOnstatus(@Param("status") Integer status, @Param("id") Long id);

    @Update( "update yqm_user set now_money = now_money + ${money} where uid = #{id}")
    void updateMoney(@Param("money") double money, @Param("id") long id);

    @Update("update yqm_user set brokerage_price = brokerage_price+ ${price} where uid = #{id}")
