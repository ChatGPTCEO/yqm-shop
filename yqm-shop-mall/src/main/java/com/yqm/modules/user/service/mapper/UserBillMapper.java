...
*/
@Repository
public interface UserBillMapper extends CoreMapper<YqmUserBill> {

    @Select("select IFNULL(sum(number),0) from yqm_user_bill " +
    ...
    @Select("SELECT DATE_FORMAT(a.create_time,'%Y-%m-%d') as addTime,a.title,a.number " +
            "FROM yqm_user_bill a INNER JOIN yqm_user u ON u.uid=a.uid WHERE a.category = 'integral'" +
    ...
    @Select("SELECT o.order_id as orderId,DATE_FORMAT(b.create_time, '%Y-%m-%d %H:%i') as time," +
            "b.number,u.avatar,u.nickname FROM yqm_user_bill b " +
            "INNER JOIN yqm_store_order o ON o.id=b.link_id " +
            "RIGHT JOIN yqm_user u ON u.uid=o.uid" +
    ...
    @Select("SELECT DATE_FORMAT(create_time,'%Y-%m') as time " +
            " FROM yqm_user_bill ${ew.customSqlSegment}")
    List<String> getBillOrderList(@Param(Constants.WRAPPER) Wrapper<YqmUserBill> userWrapper, Page page);

    @Select("SELECT DATE_FORMAT(create_time,'%Y-%m') as time,group_concat(id SEPARATOR ',') ids " +
            " FROM yqm_user_bill ${ew.customSqlSegment}")
    List<BillVo> getBillList(@Param(Constants.WRAPPER) Wrapper<YqmUserBill> userWrapper, Page page);

    @Select("SELECT DATE_FORMAT(create_time,'%Y-%m-%d %H:%i') as add_time,title,number,pm " +
            " FROM yqm_user_bill ${ew.customSqlSegment}")
    List<MUserBillDto> getUserBillList(@Param(Constants.WRAPPER) Wrapper<YqmUserBill> userWrapper);
    ...
    @Select("<script> select b.title,b.pm,b.category,b.type,b.number,b.create_time ,u.nickname " +
            "from yqm_user_bill b left join yqm_user u on u.uid=b.uid  where 1=1  "  +
    ...
    List<YqmUserBillDto> findAllByQueryCriteria(@Param("category") String category, @Param("type") String type, @Param("nickname") String nickname, @Param("pm") Integer pm, @Param("date")String date, @Param("date1")String date1,@Param("title")String title);
}
