			...
			case TYPE_1:
				...
				try {
					...
					yqmStoreCustomers.forEach(msg -> {
						if (CharSequenceUtil.isNotBlank(msg.getOpenId())) {
				...
				if (user != null) {
					...
					if (!success) {
						...
						if (ObjectUtil.isNull(resources.getFailTime())) {
							...
							// 插入流水
							billService.income(resources.getUid(), "提现失败", BillDetailEnum.CATEGORY_1.getValue(), BillDetailEnum.TYPE_4.getValue(),
									resources.getExtractPrice().doubleValue(), balance, mark, resources.getId().toString());
							// 返回提现金额
