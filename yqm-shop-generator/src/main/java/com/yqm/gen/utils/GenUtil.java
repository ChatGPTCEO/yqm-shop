...
import com.yqm.gen.domain.ColumnConfig;
...
public class GenUtil {
	...
	public static String download(List<ColumnConfig> columns, GenConfig genConfig) throws IOException {
		String tempPath =System.getProperty("java.io.tmpdir") + "yqm-shop-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
		Map<String,Object> genMap = getGenMap(columns, genConfig);
		TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
		// 生成后端代码
		List<String> templates = getAdminTemplateNames();
		for (String templateName : templates) {
			Template template = engine.getTemplate("generator/admin/"+templateName+".ftl");
			String filePath = getAdminFilePath(templateName,genConfig,genMap.get("className").toString(),tempPath + "yqm-shop" + File.separator);
			assert filePath != null;
			File file = new File(filePath);
			// 如果非覆盖生成
			if(!genConfig.getCover() && cn.hutool.core.io.FileUtil.exist(file)){
				continue;
			}
			// 生成代码
		...
		for (String templateName : templates) {
			Template template = engine.getTemplate("generator/front/"+templateName+".ftl");
			String path = tempPath + "yqm-shop-rest"  + File.separator;
	...
	private static Map<String,Object> getGenMap(List<ColumnConfig> columnInfos, GenConfig genConfig) {
		...
		// 判断是否去除表前缀
		if (org.apache.commons.lang3.StringUtils.isNotEmpty(genConfig.getPrefix())) {
			className = StringUtils.toCapitalizeCamelCase(CharSequenceUtil.removePrefix(genConfig.getTableName(),genConfig.getPrefix()));
			changeClassName = StringUtils.toCamelCase(CharSequenceUtil.removePrefix(genConfig.getTableName(),genConfig.getPrefix()));
		}
		// 保存类名
		...
		for (ColumnConfig column : columnInfos) {
			...
			// 主键存在字典
			if(org.apache.commons.lang3.StringUtils.isNotBlank(column.getDictName())){
			...
			// 表单组件类型
			listMap.put("formType", org.apache.commons.lang3.StringUtils.isNotBlank(column.getFormType()) ? column.getFormType() : "Input");
			// 小写开头的字段名称
			...
			if(org.apache.commons.lang3.StringUtils.isNotBlank(column.getDateAnnotation())){
			...
			if(!org.apache.commons.lang3.StringUtils.isBlank(column.getQueryType())){
				// 查询类型
